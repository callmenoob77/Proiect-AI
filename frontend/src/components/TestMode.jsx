...
export default function TestMode({ onBackToHome, apiBaseUrl }) {
  ...
  const [testMode, setTestMode] = useState('practice'); // 'practice' sau 'exam'
  const [timeRemaining, setTimeRemaining] = useState(null); // secunde rămase pentru exam
  ...
  const generateTest = async () => {
    ...
    try {
      ...
      // Setăm timpul pentru exam: 2 minute per întrebare
      if (testMode === 'exam') {
        setTimeRemaining(numQuestions * 120); // 2 minute = 120 secunde per întrebare
      }
    } catch (err) {
  ...
  const submitTest = async () => {
    if (loading) return; // Previne apeluri multiple
    
    setLoading(true);
    try {
      // Construim răspunsurile pentru TOATE întrebările
      // Întrebările fără răspuns vor avea valoarea ""
      const formattedAnswers = {};
      testData.questions.forEach(q => {
        const questionId = parseInt(q.id);
        formattedAnswers[questionId] = answers[q.id] || ""; // Răspuns gol pentru întrebări fără răspuns
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Nu s-a putut trimite testul');
      }
      ...
      alert('Eroare la trimiterea testului: ' + err.message);
      setLoading(false);
    }
  };
  ...
  // Cronometru pentru modul exam
  useEffect(() => {
    if (testMode === 'exam' && testState === 'taking' && timeRemaining !== null && timeRemaining > 0 && !timeExpired) {
  ...
  }, [testMode, testState, timeRemaining, timeExpired]);

  // Auto-submit când timpul expiră
  useEffect(() => {
    if (timeExpired && testMode === 'exam' && testState === 'taking' && !loading) {
      // Așteptăm un pic pentru a permite UI-ului să se actualizeze
  ...
  }, [timeExpired, testMode, testState]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-600 via-blue-600 to-cyan-500 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="font-bold text-gray-800">Practice</p>
                <p className="text-xs text-gray-600 mt-1">Fără limită de timp</p>
              </button>
              <button
              ...
              >
                ...
                <p className="font-bold text-gray-800">Examen</p>
                <p className="text-xs text-gray-600 mt-1">2 min/întrebare</p>
              </button>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {testMode === 'exam' && (
              <p className="text-center text-sm text-red-600 mt-2 font-semibold">
                ⏱️ Timp total: {numQuestions * 2} minute
              </p>
            )}
          </div>
          ...
          <button
            ...
            } text-white py-4 rounded-2xl font-bold text-lg transition-all transform hover:scale-105 shadow-xl disabled:opacity-50`}
          >
            {loading ? 'Se generează...' : `Generează Test ${testMode === 'exam' ? '🔥' : '🎯'}`}
          </button>
  ...
  if (testState === 'taking' && testData) {
    ...
    const isInputDisabled = testMode === 'exam' && timeExpired;
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-600 via-blue-600 to-cyan-500 py-8 px-4">
        ...
        {testMode === 'exam' && timeRemaining !== null && (
          <div className="max-w-4xl mx-auto mb-6">
            <div className={`rounded-2xl p-4 shadow-xl border-4 ${
              timeExpired 
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <div className="text-center">
                  <p className="text-sm text-gray-600 font-semibold">
                    {timeExpired ? 'TIMP EXPIRAT!' : 'Timp rămas'}
                  </p>
        ...
        <div className="max-w-4xl mx-auto mb-6">
          <div className={`rounded-2xl p-4 shadow-xl ${
            testMode === 'exam' ? 'bg-white border-2 border-red-300' : 'bg-white'
          }`}>
        ...
        <div className={`max-w-4xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden ${
          testMode === 'exam' ? 'border-4 border-red-500' : ''
        }`}>
          <div className={`p-6 text-white ${
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={testMode === 'exam' ? 'text-red-100 mt-1' : 'text-purple-100 mt-1'}>
                  #{currentQuestion.question_type}
                </p>
              </div>
              {testMode === 'exam' && (
                <div className="bg-white/20 px-4 py-2 rounded-xl">
                  <p className="text-sm font-semibold">MOD EXAMEN</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-8">
            <div className={`rounded-2xl p-6 mb-6 border-l-4 ${
              testMode === 'exam'
                ...
                : 'bg-gradient-to-r from-purple-50 to-blue-50 border-purple-600'
            }`}>
            ...
            <div className="space-y-4 mb-6">
              {currentQuestion.options.map((option, index) => {
                ...
                return (
                  <button
                    key={index}
                    onClick={() => !isInputDisabled && selectAnswer(currentQuestion.id, option)}
                    disabled={isInputDisabled}
                    className={`w-full text-left p-5 rounded-2xl transition-all duration-300 transform ${
                      isInputDisabled 
                      ...
                      isSelected
                        ? testMode === 'exam'
                          ? 'bg-gradient-to-r from-red-100 to-orange-100 border-2 border-red-500 shadow-lg scale-105'
            ...
            <div className="flex gap-4">
              ...
              {currentQuestionIndex === testData.questions.length - 1 ? (
                <button
                  onClick={submitTest}
                  disabled={(testMode === 'practice' && answeredCount !== testData.questions.length) || loading || isInputDisabled}
                  className={`flex-1 text-white py-3 rounded-2xl font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                    testMode === 'exam'
                      ? 'bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700'
                      : 'bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700'
                  }`}
                >
                  {loading ? 'Se trimite...' : testMode === 'exam' ? 'Trimite Examen ✓' : 'Finalizează Test ✓'}
                </button>
              ) : (
                <button
            ...
            <div className="flex flex-wrap gap-2">
              {testData.questions.map((q, index) => {
                ...
                return (
                  <button
                    key={q.id}
                    onClick={() => !isInputDisabled && goToQuestion(index)}
                    disabled={isInputDisabled}
                    className={`w-12 h-12 rounded-lg font-bold transition-all ${
                      ...
                      isCurrent
                        ? testMode === 'exam'
                          ? 'bg-gradient-to-r from-red-600 to-orange-600 text-white scale-110 shadow-lg'
